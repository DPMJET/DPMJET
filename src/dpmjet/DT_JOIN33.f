      ...
      SUBROUTINE DT_JOIN33(Idx1,Idx2,Idx3,Irej)
 
C***********************************************************************
C This subroutine joins a q-aq and a aq-q chain to a third q-qq chain.          *
...
C join chains
      ...
      if1 = idd(1,1)
      idmm1 = idd(3,2)/1000
      idmm2 = (idd(3,2)-idmm1*1000)/100
      IF ( idd(1,2).EQ.-idmm1 ) THEN
         IF ( idd(2,2).GE.idmm2 ) if2 = 1000*idd(2,2) + 100*idmm2 + 3
         IF ( idmm2.GT.idd(2,2) ) if2 = 1000*idmm2 + 100*idd(2,2) + 3
      ELSE IF ( idd(1,2).EQ.-idmm2 ) THEN
         IF ( idd(2,2).GE.idmm1 ) if2 = 1000*idd(2,2) + 100*idmm1 + 3
         IF ( idmm1.GT.idd(2,2) ) if2 = 1000*idmm1 + 100*idd(2,2) + 3
      END IF
